import { Controller, Get, Post, Put, Delete, Patch, Body, Param, Query, Req, UseGuards, HttpCode, HttpStatus } from '@nestjs/common'
...
@Controller('logistica-monedas')
export class LogisticaMonedasController {
  constructor(private readonly svc: LogisticaMonedasService) {}
  ...
  async crear(@Req() req: any, @Body() dto: CreateMonedaDto) {
  ...
  @Put(':id')
  async actualizar(@Req() req: any, @Param('id') id: string, @Body() dto: UpdateMonedaDto) {
  ...
  @Patch(':id/set-base')
  async setBase(@Req() req: any, @Param('id') id: string) {

...
export class EspecialistaController {
  constructor(private readonly svc: EspecialistaService) {}

  @Get()
  async listar(@Req() req: any, @Query('especialidad') especialidad?: string) {
  ...
  async crear(@Req() req: any, @Body() dto: CreateEspecialistaDto) {
  ...
  @Put(':id')
  async actualizar(@Req() req: any, @Param('id') id: string, @Body() dto: UpdateEspecialistaDto) {

...
export class ProveedoresController {
  ...
  private clienteIdOf(req: any): string {
    const id = req.user.clienteId ?? req.headers['x-cliente-id'] ?? null
    if (!id) throw new BadRequestException('Se requiere un contexto de cliente')
    return id
  }
  ...
  async crear(@Req() req: any, @Body() dto: CreateProveedorDto) {
  ...
  @Put(':id')
  async actualizar(@Req() req: any, @Param('id') id: string, @Body() dto: UpdateProveedorDto) {

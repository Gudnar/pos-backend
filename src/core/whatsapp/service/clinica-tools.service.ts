...
@Injectable()
export class ClinicaToolsService {
  private readonly logger = new Logger(ClinicaToolsService.name)

  constructor(
    private readonly citasSvc: CitasMedicasService,
    private readonly especialistaSvc: EspecialistaService,
    private readonly conversacionSvc: ConversacionService,
  ) {}

  async getToolDefs(clienteId: string): Promise<typeof CLINIC_TOOL_DEFS | undefined> {
    ...
    if (!servicios?.length && !especialistas?.length) return undefined
    return CLINIC_TOOL_DEFS
  }

  async getSystemAddendum(clienteId: string): Promise<string> {
    const fechaHoy = hoy()
    const diasSemana = ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado']
    const diaHoy = diasSemana[new Date().getDay()]

    const [servicios, especialistas] = await Promise.all([
      this.citasSvc.obtenerServicios(clienteId).catch(() => [] as string[]),
      this.especialistaSvc.listar(clienteId).catch(() => []),
    ])
    ...
    const especialistasStr = especialistas.length > 0
      ? especialistas.map(e =>
          `  - ID:${e.id} | ${e.nombre} | Especialidades: ${(e.especialidades || []).join(', ') || 'general'}`
        ).join('\n')
...
- CIERRE: cuando el paciente se despida (ej: "gracias", "hasta luego", "listo") y no haya nada pendiente, llama cerrar_conversacion DESPUÉS de enviar tu mensaje de despedida.`
  ...
  async ejecutar(nombre: string, input: Record<string, any>, ctx: ToolContext): Promise<any> {
  ...
  private async _obtenerServicios(ctx: ToolContext) {
    const [servicios, especialistas] = await Promise.all([
      this.citasSvc.obtenerServicios(ctx.clienteId),
      this.especialistaSvc.listar(ctx.clienteId),
    ])

    const especialistasPorServicio: Record<string, Array<{ id: string; nombre: string; descripcion?: string }>> = {}

    for (const servicio of servicios) {
      const svcLower = servicio.toLowerCase()
      const match = especialistas.filter(e =>
        e.activo &&
        (e.especialidades || []).some(esp =>
          esp.toLowerCase().includes(svcLower) || svcLower.includes(esp.toLowerCase()),
        ),
      )
  ...
  private async _listarEspecialistas(especialidad: string | undefined, ctx: ToolContext) {
  ...
  private async _consultarDisponibilidad(fecha: string, especialistaId: string | undefined, ctx: ToolContext) {
  ...
  private async _programarConsulta(input: Record<string, any>, ctx: ToolContext) {
    const { pacienteNombre, fecha, horaInicio, servicio, notas, especialistaId } = input
    const pacienteTelefono: string = input.pacienteTelefono || ctx.from
    ...
    let especialistaNombre: string | undefined
    if (especialistaId) {
  ...
  private async _consultarConsultasPaciente(telefono: string, ctx: ToolContext) {
  ...
  private async _reprogramarConsulta(
    consultaId: string,
    nuevaFecha: string,
  ...
  private async _cancelarConsulta(consultaId: string, motivo: string | undefined, ctx: ToolContext) {
    await this.citasSvc.actualizar(ctx.clienteId, consultaId, {
  ...
  private async _cerrarConversacion(motivo: string | undefined, ctx: ToolContext) {
